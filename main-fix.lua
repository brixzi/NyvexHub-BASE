...
do
    ...
    function Animation.Apply(theme, root, shineEnabled)
        ...
        for _, obj in ipairs(_gradients) do
            pcall(function() obj:SetAttribute("_ShineAnimated", true) end)
        end
...
local aa = {
        ...
        function o.SelectTab(p, q)
            ...
            task.spawn(
                function()
                    ...
                    r.ContainerPosMotor:setGoal(l(100, {frequency = 12}))
                    task.wait(0.12)
            ...
            csSharedConn = game:GetService("RunService").Heartbeat:Connect(function(dt)
                ...
                for key, data in next, csEntries do
                    ...
                    if inst and inst.Parent then
                        hasAny = true
                        if not inst:GetAttribute("_ShineAnimated") then
                            if data._wasShineAnimated then
                                data._wasShineAnimated = nil
                                data.t = 0
                            end
    ...
    [59] = function()
        ...
        function c.New(d, e, f)
            ...
            local _inGroupContainer = d.Type == "Group" or d.Type == "HGroup" or d.Type == "VGroup"
            local _xOffset = _inGroupContainer and 0 or -16
            local wrap = u("Frame", {
                Size = UDim2.new(1, _xOffset, 0, 150),
                BackgroundTransparency = 1,
                ClipsDescendants = true,
                Parent = parent,
            })
            local function _recalcAspect()
                ...
                if w > 0 and ratioNum and ratioNum > 0 then
                    wrap.Size = UDim2.new(1, _xOffset, 0, math.floor(w / ratioNum))
                end
            end
    ...
    [60] = function()
        ...
        function c.New(d, e, f)
            ...
            local _inGroupContainerVid = d.Type == "Group" or d.Type == "HGroup" or d.Type == "VGroup"
            local _xOffsetVid = _inGroupContainerVid and 0 or -16
            local wrap = u("Frame",{
                Size=UDim2.new(1,_xOffsetVid,0,180),
            ...
            local function _recalcAspect2()
                ...
                if w > 0 and ratioNum2 and ratioNum2 > 0 then
                    wrap.Size = UDim2.new(1, _xOffsetVid, 0, math.floor(w / ratioNum2))
                end
            end
    ...
    [65] = function()
        ...
        function c.New(d, e, f)
            ...
            local _inGroupContainerAud = d.Type == "Group" or d.Type == "HGroup" or d.Type == "VGroup"
            local _xOffsetAud = _inGroupContainerAud and 0 or -16
            local wrap = u("Frame",{
                Size=UDim2.new(1,_xOffsetAud,0,wrapHeight),
                BackgroundTransparency=0.9,
                BorderSizePixel=0,
                Parent=parent,
                ThemeTag={BackgroundColor3="Element"},
